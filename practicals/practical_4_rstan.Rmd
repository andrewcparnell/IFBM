...
We are going to focus on the `prostate` data set in the `data` folder. We're going to use `rstan` throughout, though you're welcome to try some of the models in `rstanarm`.

## The prostate data set

The `prostate` data set can be loaded in and explored with:
```{r}
prostate = read.csv('https://raw.githubusercontent.com/andrewcparnell/IFBM/master/data/prostate.csv')
head(prostate)
```
...
To create the WAIC value you first need to re-run your model, adding in a `generated quantities` block which outputs the log likelihood, just like in the lectures.
