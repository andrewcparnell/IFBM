...
- The `lme4` package creates these everywhere, but the later packages (`rstan` and `rstanarm`) do not use them
...
- Later in the course we will meet a method (called HMC) which finds the best values, also gets the uncertainty, and doesn't require the assumption of parameter normality
