---
title: 'Class 13: Variable selection and non-parametrics'
...
- Be able to fit shrinkage and selection models in Stan
...
- Some steps to model development:
    ...
    1. Fit the model in `rstan` or `rstanarm`
    1. Get the WAIC/LOO and create a posterior predictive plot
...
- This is creating a random effect by borrowing strength across covariates rather than across observations
...
```{r, message=FALSE}
plot(stan_run, pars = c('beta','sigma_b'))
```
...
- The plot of the density looks even more spiked than that of the Lasso (though possibly due to poor convergence)
...
```{r, message=FALSE}
plot(stan_run, pars = c('beta','sigma','tau'))
```


## Final notes on Shrinkage

- If we're concerned with predictive performance we can assess these via cross-validation, or compare them using WAIC/LOO
...
- Unfortunately you cannot fit this model in Stan, as it does not allow for discrete parameters $I_j$.
...
- This is simple to fit in Stan using much of the code we have already seen
